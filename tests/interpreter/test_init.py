...
class TestInterpreterUtils(unittest.TestCase):
    ...
    def test_search_datetime(self):
        """Test searching datetime strings.

        Trying: search datetime strings
        Expecting: list of datetime string spans
        """
        ...
        for line in (now, *keywords):
            self.assertCountEqual(
                src.interpreter.search_datetime(
                    f"{line} {line}",
                    keywords=keywords,
                ),
                ((0, len(line)), (len(line) + 1, 2 * len(line) + 1)),
            )
        self.assertFalse(
            list(src.interpreter.search_datetime("", keywords=keywords)),
        )
